...
namespace Condominio.Repositories
{
    public class RegistroAccesoRepository : IRegistroAccesoRepository
    {
        ...
        {
            using IDbConnection db = new OracleConnection(_conn);
            // CORREGIDO: :Punto_Accedo → :Punto_Acceso
            const string sql = @"
                INSERT INTO REGISTRO_ACCESO (
                    TIPO_MOVIMIENTO, TIPO_PERSONA, ID_RESIDENTE, ID_VISITANTE,
                    ID_EMPLEADO, NOMBRE_PERSONA, DPI_PERSONA, PLACA_VEHICULO,
                    PUNTO_ACCESO, AUTORIZADO_POR, OBSERVACIONES
                ) VALUES (
                    :Tipo_Movimiento, :Tipo_Persona, :Id_Residente, :Id_Visita,
                    :Id_Empleado, :Nombre_Persona, :Dpi_Persona, :Placa_Vehiculo,
                    :Punto_Acceso, :Autorizado_Por, :Observaciones
                )";
